...
void WaveReconV2::forward(const bool                         add,
  ...
  std::shared_ptr<float2D> slsq =
    ((std::dynamic_pointer_cast<float2DReg>(_slsqModel))->_mat);
  ...
  for (int i3 = 0; i3 < n3; i3++) {
    for (int i2 = 0; i2 < n2; i2++) {
      for (int i1 = 0; i1 < n1; i1++) {
        (*t0)[i3][i2][i1] = (*t0)[i3][i2][i1] * (*slsq)[i3][i2];
...
void WaveReconV2::adjoint(const bool                         add,
  ...
  for (int i3 = 0; i3 < n3; i3++) {
    for (int i2 = 0; i2 < n2; i2++) {
      for (int i1 = 0; i1 < n1; i1++) {
        (*t0)[i3][i2][i1] = (*t0)[i3][i2][i1] * (*slsq)[i3][i2]; // temp1
        // =
        // (d^2/dt^2)*(W(data))s^2
      }
    }
