  ...
  std::shared_ptr<float4D> b =
    ...
    for (int is = 0; is < n4; is++) {
      for (int it = 0; it < n3; it++) {
        for (int ix = 0; ix < n2; ix++) {
          for (int iz = 0; iz < n1; iz++) {
            (*b)[is][it ][ix + _laplOrder][iz+ _laplOrder] = (*d)[is][it][ix][iz];
          }
        }
      }
    }
...
/**
   Many 2d slices case over multiple shots
 */
Laplacian2d_multi_exp_complex::Laplacian2d_multi_exp_complex(const std::shared_ptr<complex4DReg>model,
                         const std::shared_ptr<complex4DReg>data) {
  // ensure dimensions match
  assert(model->getHyper()->getAxis(1).n == data->getHyper()->getAxis(1).n);
  assert(model->getHyper()->getAxis(2).n == data->getHyper()->getAxis(2).n);
  assert(model->getHyper()->getAxis(3).n == data->getHyper()->getAxis(3).n);
  ...
  buffer.reset(new SEP::complex4DReg(data->getHyper()->getAxis(1).n+ 2 * _laplOrder,
                                    data->getHyper()->getAxis(2).n + 2 * _laplOrder,
                                    data->getHyper()->getAxis(3).n,
                                    data->getHyper()->getAxis(4).n));
  buffer->zero();
}
void Laplacian2d_multi_exp_complex::forward(const bool                         add,
                          const std::shared_ptr<SEP::complex4DReg>model,
                          std::shared_ptr<SEP::complex4DReg>      data) const {
  assert(checkDomainRange(model, data));
  if (!add) data->scale(0.);

  std::shared_ptr<complex4D> b =
    ((std::dynamic_pointer_cast<complex4DReg>(buffer))->_mat);
...
void Laplacian2d_multi_exp_complex::adjoint(const bool                         add,
