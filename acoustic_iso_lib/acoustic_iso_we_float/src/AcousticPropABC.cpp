  ...
  const int                              velPadz)
{
  ...
  // initialize C4 op
  // compute _C4f. This never changes so save as private variable _C4f
  _C4.reset(new waveform::C4_2DCube(model,
                                    data,
                                    velPadded,
                                    velPadx,
                                    velPadz,
  ...
  // std::cerr << "t=1 max source=" << source->absMax() << std::endl;
  // std::cerr << "t=1 max _Cf4=" << _C4f->absMax() << std::endl << std::endl;

  // initialize C6 op
  _C6.reset(new waveform::C6(
              _temp0,
              _temp1,
              velPadded,
              velPadx,
...
void AcousticPropABC::forward(const bool                         add,
  ...
  for (int i3 = 0; i3 < n3; i3++) {
    for (int i2 = 0; i2 < n2; i2++) {
      // (*d)[i3][i2][0] += 0;
      (*_temp0->_mat)[i3][i2] = double((*_temp0->_mat)[i3][i2]) +
                                double((*d)[i3][i2][0]);
    }
  }
  ...
  for (int i3 = 0; i3 < n3; i3++) {
    for (int i2 = 0; i2 < n2; i2++) {
      (*_temp1->_mat)[i3][i2] = double((*_temp1->_mat)[i3][i2]) -
                                double((*_C4f->_mat)[i3][i2][0]);
      (*d)[i3][i2][1] = double((*d)[i3][i2][1])   +
                        double((*_temp1->_mat)[i3][i2]);
    }
  }
  ...
  for (int it = 2; it < _nt; it++) {
    ...
    for (int i3 = 0; i3 < n3; i3++) {
      for (int i2 = 0; i2 < n2; i2++) {
        (*_temp2->_mat)[i3][i2] = double((*_temp2->_mat)[i3][i2]) -
                                  double((*_C4f->_mat)[i3][i2][it - 1]);
        (*d)[i3][i2][it] = double((*d)[i3][i2][it])  +
                           double((*_temp2->_mat)[i3][i2]);
      }
    }
...
void AcousticPropABC::adjoint(const bool                         add,
  ...
  for (int i3 = 0; i3 < n3; i3++) {
    for (int i2 = 0; i2 < n2; i2++) {
      (*_temp1->_mat)[i3][i2] = double((*_temp1->_mat)[i3][i2]) -
                                double((*_C4f->_mat)[i3][i2][_nt - 1]);
      (*m)[i3][i2][_nt - 2] = double((*m)[i3][i2][_nt - 2]) +
                              double((*_temp1->_mat)[i3][i2]);
    }
  }
