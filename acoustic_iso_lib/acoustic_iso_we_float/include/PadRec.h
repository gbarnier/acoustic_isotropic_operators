...
class PadRec : public Operator<SEP::float2DReg, SEP::float3DReg> {
public:
  /**
  ...
  */
  PadRec(const std::shared_ptr<SEP::float2DReg> model,
    const std::shared_ptr<SEP::float3DReg> data, const int s2);
