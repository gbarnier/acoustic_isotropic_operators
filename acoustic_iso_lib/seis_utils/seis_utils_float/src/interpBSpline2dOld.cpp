...
// B-Spline weights
float interpBSpline2d::bspline1d(int iControl, float uValue, int order, float dk, int nControlPoints, std::shared_ptr<float1DReg> knots) {

    // Compute shift
    float s=uValue-(*knots->_mat)[iControl];
    float weight=0;
    ...
    /********************************* Order 1 ********************************/
        ...
        // First control point (float knot)
        if (iControl==0) {
            if (s>=0 && s<dk){
                weight=1.0-s/dk;
            }
        ...
        // Last control point (float knots)
