...
if __name__ == "__main__":
	...
	if nPSF3 != 1 and nExt == 1:
		raise ValueError("ERROR! nPSF3 must be 1 for non-extended images")
	if nPSF3 == 1 and nExt != 1:
		raise ValueError("ERROR! nPSF3 must be different than 1 for extended images")
	if nPSF1 <= 0 or nPSF2 <= 0 or nPSF3 <= 0:
		raise ValueError("ERROR! nPSF must be positive")
	...
	zz,xx,ee = np.meshgrid(PSF1_pos,PSF2_pos,PSF3_pos,indexing='ij',sparse=True)
