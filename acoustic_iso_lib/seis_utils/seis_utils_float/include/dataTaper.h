...
class dataTaper : public Operator<SEP::float3DReg, SEP::float3DReg> {
	...
	private:
		...
		float _maxOffset, _expOffset, _taperWidthOffset;
	...
	public:
		...
		dataTaper(float t0, float velMute, float expTime, float taperWidthTime, std::string moveout, int reverseTime, float maxOffset, float expOffset, float taperWidthOffset, int reverseOffset, std::shared_ptr<SEP::hypercube> dataHyper);

		// Constructor for time only tapering only
		dataTaper(float t0, float velMute, float expTime, float taperWidthTime, std::shared_ptr<SEP::hypercube> dataHyper, std::string moveout, int reverseTime);

		// Constructor for offset tapering only
		dataTaper(float maxOffset, float _expOffset, float _taperWidthOffset, std::shared_ptr<SEP::hypercube> dataHyper, int _reverseOffset);

		// Constructor for no tapering
		dataTaper(std::shared_ptr<SEP::hypercube> dataHyper);
