...
class dataTaper : public Operator<SEP::float3DReg, SEP::float3DReg> {
	...
	public:
		...
		dataTaper(float t0, float velMute, float expTime, float taperWidthTime, std::string moveout, int reverseTime, float maxOffset, float expOffset, float taperWidthOffset, int reverseOffset, std::shared_ptr<SEP::hypercube> dataHyper, float taperEndTraceWidth, int streamers);
		...
		dataTaper(float maxOffset, float _expOffset, float _taperWidthOffset, std::shared_ptr<SEP::hypercube> dataHyper, int _reverseOffset, float taperEndTraceWidth, int streamers);
