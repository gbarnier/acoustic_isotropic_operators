...
dataTaperDouble::dataTaperDouble(double maxOffset, double exp, double taperWidth, std::shared_ptr<SEP::hypercube> dataHyper, std::string muteType){
	...
	if (_muteType == "offset"){
		...
		for (int iShot=0; iShot<_nShot; iShot++){
			...
			double s=_xMinShot+iShot*_dShot;
			...
			double xMinRecMute1=std::max(s-_maxOffset, _xMinRec);
			double xMaxRecMute1=std::min(s+_maxOffset, _xMaxRec);

			// Compute cutoff position [km] of the receivers from each side of the source
			// Beyond that cutoff, zero out the traces
			double xMinRecMute2=std::max(s-_maxOffset-_taperWidth, _xMinRec);
			double xMaxRecMute2=std::min(s+_maxOffset+_taperWidth, _xMaxRec);
			...
			for (int iRec=0; iRec<_nRec; iRec++){
				...
				if((iRec>=ixMinRecMute2) && (iRec<ixMinRecMute1)){
					double argument=1.0*(iRec-ixMinRecMute2)/(ixMinRecMute1-ixMinRecMute2);
					double weight=pow(sin(3.14159/2.0*argument), _exp);
					for (int its=0; its<_dataHyper->getAxis(1).n; its++){
				...
				if((iRec>ixMaxRecMute1) && (iRec<=ixMaxRecMute2)){
					double argument=1.0*(iRec-ixMaxRecMute1)/(ixMaxRecMute2-ixMaxRecMute1);
					double weight=pow(cos(3.14159/2.0*argument), _exp);
					for (int its=0; its<_dataHyper->getAxis(1).n; its++){
						(*_taperMask->_mat)[iShot][iRec][its] = weight;
					}
				}
...
void dataTaperDouble::forward(const bool add, const std::shared_ptr<double3DReg> model, std::shared_ptr<double3DReg> data) const{
...
void dataTaperDouble::adjoint(const bool add, std::shared_ptr<double3DReg> model, const std::shared_ptr<double3DReg> data) const{
