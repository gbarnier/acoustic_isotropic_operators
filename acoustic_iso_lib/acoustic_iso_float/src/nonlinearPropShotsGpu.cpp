...
// Cosntructor
nonlinearPropShotsGpu::nonlinearPropShotsGpu(std::shared_ptr<SEP::float2DReg> vel, std::shared_ptr<paramObj> par, std::vector<std::shared_ptr<deviceGpu>> sourcesVector, std::vector<std::shared_ptr<deviceGpu>> receiversVector) {
...
// Forward
void nonlinearPropShotsGpu::forward(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {
	...
	for (int iShot=0; iShot<_nShot; iShot++){
		...
		if(constantSrcSignal == 1) {
			memcpy(modelSliceVector[iGpu]->getVals(), &(model->getVals()[0]), sizeof(float)*hyperModelSlice->getAxis(1).n*hyperModelSlice->getAxis(2).n);
		} else {
			memcpy(modelSliceVector[iGpu]->getVals(), &(model->getVals()[iShot*hyperModelSlice->getAxis(1).n*hyperModelSlice->getAxis(2).n]), sizeof(float)*hyperModelSlice->getAxis(1).n*hyperModelSlice->getAxis(2).n);
		}
...
void nonlinearPropShotsGpu::forwardWavefield(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) {
...
// Adjoint
void nonlinearPropShotsGpu::adjoint(const bool add, std::shared_ptr<float3DReg> model, const std::shared_ptr<float3DReg> data) const {
...
void nonlinearPropShotsGpu::adjointWavefield(const bool add, std::shared_ptr<float3DReg> model, const std::shared_ptr<float3DReg> data) {
