...
void initTomoExtGpu(float dz, float dx, int nz, int nx, int nts, float dts, int sub, int minPad, int blockSize, float alphaCos, int nExt, int leg1, int leg2, int nGpu, int iGpuId, int iGpuAlloc){
	...
	if (iGpuId == iGpuAlloc) {
		...
		dev_tomoSrcWavefieldDt2 = new float*[nGpu];
		dev_tomoSecWavefield1 = new float*[nGpu];
		dev_tomoSecWavefield2 = new float*[nGpu];
...
void allocateTomoExtShotsGpu(float *vel2Dtw2, float *reflectivityScale, int iGpu, int iGpuId){
	...
	cuda_call(cudaMemcpy(vel2Dtw2, dev_vel2Dtw2[iGpu], host_nz*host_nx*sizeof(float), cudaMemcpyDeviceToHost));
	...
	cuda_call(cudaMalloc((void**) &dev_modelTomo[iGpu], host_nz*host_nx*sizeof(float)));

	// Allocate and copy from host to device extended reflectivity
	cuda_call(cudaMalloc((void**) &dev_extReflectivity[iGpu], host_nz*host_nx*host_nExt*sizeof(float)));

	// Allocate source wavefield - The allocation of secondary sources are done inside the function implementation
	cuda_call(cudaMalloc((void**) &dev_tomoSrcWavefieldDt2[iGpu], host_nz*host_nx*host_nts*sizeof(float))); // We store the source wavefield

	// Allocate the auxiliary wavefield (either scattered for the fwd, or receiver for the adjoint)
	cuda_call(cudaMalloc((void**) &dev_tomoSecWavefield1[iGpu], host_nz*host_nx*host_nts*sizeof(float))); // We store the source wavefield
}
...
/********************************** Normal ************************************/
// Time-lag extension
void tomoTimeShotsFwdGpu(float *model, float *dataRegDts, float *extReflectivity, float *sourcesSignals, int *sourcesPositionReg, int nSourcesReg, int *receiversPositionReg, int nReceiversReg, float *tomoSrcWavefieldDt2, float *tomoSecWavefield1, float *tomoSecWavefield2, int sloth, int iGpu, int iGpuId, int saveWavefield){
...
/********************************** Normal ************************************/
// Time-lag extension
void tomoTimeShotsAdjGpu(float *model, float *dataRegDts, float *extReflectivity, float *sourcesSignals, int *sourcesPositionReg, int nSourcesReg, int *receiversPositionReg, int nReceiversReg, float *tomoSrcWavefieldDt2, float *tomoSecWavefield1, float *tomoSecWavefield2, int sloth, int iGpu, int iGpuId, int saveWavefield){
