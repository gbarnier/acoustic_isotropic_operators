...
// Source wavefield
void computeWemvaSrcWfldDt2(float *dev_sourcesIn, float *dev_wavefieldOut, int *dev_sourcesPositionsRegIn, int nSourcesRegIn, dim3 dimGridIn, dim3 dimBlockIn, int iGpu);
// Receiver wavefield
void computeWemvaRecWfld(float *dev_dataIn, float *dev_wavefieldOut, int *dev_receiversPositionsRegIn, dim3 dimGridIn, dim3 dimBlockIn, int nBlockDataIn, int iGpu);

// Forward time
void computeWemvaLeg1TimeFwd(float *dev_modelWemvaIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_wemvaExtImageOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGridInExtIn, dim3 dimBlockInExtIn, int iGpu, int saveWavefield);
void computeWemvaLeg2TimeFwd(float *dev_modelWemvaIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_wemvaExtImageOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGridInExtIn, dim3 dimBlockInExtIn, int iGpu, int saveWavefield);

// Forward offset
void computeWemvaLeg1OffsetFwd(float *dev_modelWemvaIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_wemvaExtImageOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGridExtIn, dim3 dimBlockExtIn, int iGpu, int saveWavefield);
void computeWemvaLeg2OffsetFwd(float *dev_modelWemvaIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_wemvaExtImageOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGridExtIn, dim3 dimBlockExtIn, int iGpu, int saveWavefield);

// Adjoint time
void computeWemvaLeg1TimeAdj(float *dev_wemvaExtImageIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_modelWemvaOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, int saveWavefield);
void computeWemvaLeg2TimeAdj(float *dev_wemvaExtImageIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_modelWemvaOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, int saveWavefield);

// Adjoint offset
void computeWemvaLeg1OffsetAdj(float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_modelWemvaOut, float *dev_wavefield1Out, float *dev_extReflectivityIn, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, int saveWavefield);
void computeWemvaLeg2OffsetAdj(float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_modelWemvaOut, float *dev_wavefield1Out, float *dev_extReflectivityIn, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, int saveWavefield);
...
void initWemvaExtGpu(float dz, float dx, int nz, int nx, int nts, float dts, int sub, int minPad, int blockSize, float alphaCos, int nExt, int leg1, int leg2, int nGpu, int iGpuId, int iGpuAlloc){
	...
	if (iGpuId == iGpuAlloc) {
		...
		// Time slices for FD stepping at coarse grid
		dev_scatLeft = new float*[nGpu];
		dev_scatRight = new float*[nGpu];
		dev_scatTemp1 = new float*[nGpu];

		// Time slices for FD stepping at coarse grid before second order time derivative
		dev_ss0 = new float*[nGpu];
		dev_ss1 = new float*[nGpu];
		dev_ss2 = new float*[nGpu];
		dev_ssTemp2 = new float*[nGpu];

		// wemva Data
		dev_wemvaDataRegDts = new float*[nGpu];
		...
		dev_modelWemva = new float*[nGpu];

		// Extended image ("data") for wemva
		dev_wemvaExtImage = new float*[nGpu];

		// Source and secondary wavefields
		dev_wemvaSrcWavefieldDt2 = new float*[nGpu];
		dev_wemvaSecWavefield1 = new float*[nGpu];
		dev_wemvaSecWavefield2 = new float*[nGpu];
...
void allocateWemvaExtShotsGpu(float *vel2Dtw2, float *reflectivityScale, int iGpu, int iGpuId){
	...
	cuda_call(cudaMalloc((void**) &dev_p0[iGpu], host_nz*host_nx*sizeof(float))); // Fine time sampling temporary slices
	cuda_call(cudaMalloc((void**) &dev_p1[iGpu], host_nz*host_nx*sizeof(float)));

  	cuda_call(cudaMalloc((void**) &dev_ssLeft[iGpu], host_nz*host_nx*sizeof(float))); // Coarse time sampling temporary slices
  	cuda_call(cudaMalloc((void**) &dev_ssRight[iGpu], host_nz*host_nx*sizeof(float)));

	cuda_call(cudaMalloc((void**) &dev_scatLeft[iGpu], host_nz*host_nx*sizeof(float))); // Coarse time sampling temporary slices
  	cuda_call(cudaMalloc((void**) &dev_scatRight[iGpu], host_nz*host_nx*sizeof(float)));

	cuda_call(cudaMalloc((void**) &dev_ss0[iGpu], host_nz*host_nx*sizeof(float)));
	cuda_call(cudaMalloc((void**) &dev_ss1[iGpu], host_nz*host_nx*sizeof(float)));
	cuda_call(cudaMalloc((void**) &dev_ss2[iGpu], host_nz*host_nx*sizeof(float)));

	// Allocate non-extended model
	cuda_call(cudaMalloc((void**) &dev_modelWemva[iGpu], host_nz*host_nx*sizeof(float)));

	// Allocate non-extended model
	cuda_call(cudaMalloc((void**) &dev_wemvaExtImage[iGpu], host_nz*host_nx*host_nExt*sizeof(float)));

	// Allocate source and receiver wavefields
	cuda_call(cudaMalloc((void**) &dev_wemvaSrcWavefieldDt2[iGpu], host_nz*host_nx*host_nts*sizeof(float))); // We store the source wavefield
	cuda_call(cudaMalloc((void**) &dev_wemvaSecWavefield1[iGpu], host_nz*host_nx*host_nts*sizeof(float))); // We store the source wavefield
}
...
void wemvaTimeShotsFwdGpu(float *model, float *wemvaExtImage, float *sourcesSignals, int *sourcesPositionReg, int nSourcesReg, float *wemvaDataRegDts, int *receiversPositionReg, int nReceiversReg, float *wemvaSrcWavefieldDt2, float *wemvaSecWavefield1, float *wemvaSecWavefield2, int iGpu, int iGpuId, int saveWavefield){
	...
	// Receivers signals
  	cuda_call(cudaMalloc((void**) &dev_wemvaDataRegDts[iGpu], nReceiversReg*host_nts*sizeof(float))); // Allocate sources signals on device
	cuda_call(cudaMemcpy(dev_wemvaDataRegDts[iGpu], wemvaDataRegDts, nReceiversReg*host_nts*sizeof(float), cudaMemcpyHostToDevice)); // Copy sources signals on device
	...
	if (saveWavefield == 1) {cuda_call(cudaMemcpy(wemvaSrcWavefieldDt2, dev_wemvaSrcWavefieldDt2[iGpu], host_nz*host_nx*host_nts*sizeof(float), cudaMemcpyDeviceToHost));}
	...
	if (saveWavefield == 1) {cuda_call(cudaMemcpy(wemvaSecWavefield1, dev_wemvaSecWavefield1[iGpu], host_nz*host_nx*host_nts*sizeof(float), cudaMemcpyDeviceToHost));}

	/************************************************************************************/
	/***************************** Preliminary steps ************************************/
	/************************************************************************************/
	// Copy + scale model (background perturbation)
	cuda_call(cudaMemcpy(dev_modelWemva[iGpu], model, host_nz*host_nx*sizeof(float), cudaMemcpyHostToDevice));
	...
	if (saveWavefield == 1) {cuda_call(cudaMalloc((void**) &dev_wemvaSecWavefield2[iGpu], host_nz*host_nx*host_nts*sizeof(float)));}

    // Initialize extended image ("data") to zero
	cuda_call(cudaMemset(dev_wemvaExtImage[iGpu], 0, host_nz*host_nx*host_nExt*sizeof(float)));
	...
	if (host_leg1 == 1){
		...
		if (saveWavefield == 1) {
			cuda_call(cudaMemcpy(wemvaSecWavefield2, dev_wemvaSecWavefield2[iGpu], host_nz*host_nx*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		}
	}
	...
	// Copy data to host
	cuda_call(cudaMemcpy(wemvaExtImage, dev_wemvaExtImage[iGpu], host_nz*host_nx*host_nExt*sizeof(float), cudaMemcpyDeviceToHost));
...
void wemvaTimeShotsAdjGpu(float *model, float *wemvaExtImage, float *sourcesSignals, int *sourcesPositionReg, int nSourcesReg, float *wemvaDataRegDts, int *receiversPositionReg, int nReceiversReg, float *wemvaSrcWavefieldDt2, float *wemvaSecWavefield1, float *wemvaSecWavefield2, int iGpu, int iGpuId, int saveWavefield){
	...
	cuda_call(cudaMemcpy(dev_wemvaExtImage[iGpu], wemvaExtImage, host_nz*host_nx*host_nExt*sizeof(float), cudaMemcpyHostToDevice));
	kernel_exec(scaleReflectivityExt<<<dimGridExt, dimBlockExt>>>(dev_wemvaExtImage[iGpu], dev_reflectivityScale[iGpu], dev_vel2Dtw2[iGpu]));
	...
	cuda_call(cudaMemset(dev_modelWemva[iGpu], 0, host_nz*host_nx*sizeof(float)));
	...
	// Copy data to host
	cuda_call(cudaMemcpy(model, dev_modelWemva[iGpu], host_nz*host_nx*sizeof(float), cudaMemcpyDeviceToHost));
...
void computeWemvaLeg1TimeFwd(float *dev_modelWemvaIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_wemvaExtImageOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGridExtIn, dim3 dimBlockExtIn, int iGpu, int saveWavefield){
...
void computeWemvaLeg2TimeFwd(float *dev_modelWemvaIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_wemvaExtImageOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, dim3 dimGridExtIn, dim3 dimBlockExtIn, int iGpu, int saveWavefield){
...
void computeWemvaLeg1OffsetAdj(float *dev_wemvaExtImageIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_modelWemvaOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, int saveWavefield){
...
void computeWemvaLeg2OffsetAdj(float *dev_wemvaExtImageIn, float *dev_wemvaSrcWavefieldDt2In, float *dev_wemvaRecWavefieldIn, float *dev_modelWemvaOut, float *dev_wavefield1Out, dim3 dimGridIn, dim3 dimBlockIn, int iGpu, int saveWavefield){
