...
interpSplineLinear1d::interpSplineLinear1d(std::shared_ptr<int1DReg> modelGridPos, axis gridAxis){
	...
	_interpFilter = std::make_shared<float1DReg>(_nInterval); // Interpolation filter

	// Compute filter coefficients
	for (int iData=0; iData<_nData; iData++){

		// Compute position of data point
		float d=_o1+(*_modelGridPos->_mat)[iData]*_d1;
...
void interpSplineLinear1d::forward(const bool add, const std::shared_ptr<float2DReg> model, std::shared_ptr<float2DReg> data) const {
...
void interpSplineLinear1d::adjoint(const bool add, std::shared_ptr<float2DReg> model, const std::shared_ptr<float2DReg> data) const {
