#ifndef TOMO_EXT_SHOTS_GPU_H
#define TOMO_EXT_SHOTS_GPU_H 1
...
class tomoExtShotsGpu : public Operator<SEP::float2DReg, SEP::float3DReg> {
	...
	public:
		...
		tomoExtShotsGpu(std::shared_ptr<SEP::float2DReg> vel, std::shared_ptr<paramObj> par, std::vector<std::shared_ptr<deviceGpu>> sourcesVector, std::vector<std::shared_ptr<SEP::float2DReg>> sourcesSignalsVector, std::vector<std::shared_ptr<deviceGpu>> receiversVector, std::shared_ptr<SEP::float3DReg> reflectivityExt);
		...
		void setReflectivityExt(std::shared_ptr<SEP::float3DReg> reflectivityExt){ _reflectivityExt = reflectivityExt; }
