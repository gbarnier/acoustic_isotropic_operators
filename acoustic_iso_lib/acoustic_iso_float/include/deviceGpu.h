...
class deviceGpu : public Operator<SEP::float2DReg, SEP::float2DReg> {
	...
	public:
		...
		deviceGpu(const std::shared_ptr<float1DReg> zCoord, const std::shared_ptr<float1DReg> xCoord, const std::shared_ptr<float2DReg> vel, int &nt, int dipole, float zDipoleShift, float xDipoleShift);
		deviceGpu(const std::vector<int> &zGridVector, const std::vector<int> &xGridVector, const std::shared_ptr<float2DReg> vel, int &nt, int dipole, int zDipoleShift, int xDipoleShift);
		deviceGpu(const int &nzDevice, const int &ozDevice, const int &dzDevice , const int &nxDevice, const int &oxDevice, const int &dxDevice, const std::shared_ptr<float2DReg> vel, int &nt, int dipole, int zDipoleShift, int xDipoleShift);
