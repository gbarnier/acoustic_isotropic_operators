...
int main(int argc, char **argv) {
	...
	std::shared_ptr<double2DReg> model1Float, model2Float; // Model
	std::shared_ptr<double3DReg> data1Float, data2Float; // Data
	...
	if (adj == 1) {
		...
		/* Allocate and read model */
		model1Float = std::make_shared<double2DReg>(velHyper);
	...
	/* Wavefields */
	std::shared_ptr<double3DReg> srcWavefield1Float, secWavefield1Float;
	...
	/* Create Born object */
	std::shared_ptr<BornShotsGpu> object1(new BornShotsGpu(velFloat, par, sourcesVector, sourcesSignalsVector, receiversVector));
