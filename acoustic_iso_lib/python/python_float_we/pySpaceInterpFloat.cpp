...
PYBIND11_MODULE(pySpaceInterpFloat, clsGeneric) {
  //Necessary to redirect std::cout into python stdout
    ...
    ;
		...
		py::class_<spaceInterp_multi_exp_complex, std::shared_ptr<spaceInterp_multi_exp_complex>>(clsGeneric,"spaceInterp_multi_exp_complex")  //
			.def(py::init<const std::shared_ptr<float1DReg>, const std::shared_ptr<float1DReg>, const std::shared_ptr<float1DReg>, const std::shared_ptr<SEP::hypercube>, int&, std::string , int>(),"Initlialize spaceInterp_multi_exp_complex")

			.def("forward",(void (spaceInterp_multi_exp_complex::*)(const bool, const std::shared_ptr<complex2DReg>, std::shared_ptr<complex2DReg>)) &spaceInterp_multi_exp_complex::forward,"Forward")

			.def("adjoint",(void (spaceInterp_multi_exp_complex::*)(const bool, std::shared_ptr<complex2DReg>, const std::shared_ptr<complex2DReg>)) &spaceInterp_multi_exp_complex::adjoint,"Adjoint")

			.def("dotTest",(bool (spaceInterp_multi_exp_complex::*)(const bool, const float)) &spaceInterp_multi_exp_complex::dotTest,"Dot-Product Test")

			.def("getNDeviceReg",(int (spaceInterp_multi_exp_complex::*)())&spaceInterp_multi_exp_complex::getNDeviceReg,"Get number of regular devices")

			.def("getNDeviceIrreg",(int (spaceInterp_multi_exp_complex::*)())&spaceInterp_multi_exp_complex::getNDeviceIrreg,"Get number of regular devices")

			.def("getRegPosUniqueVector",(std::vector<std::vector<int>> (spaceInterp_multi_exp_complex::*)())&spaceInterp_multi_exp_complex::getRegPosUniqueVector,"Get vector of unique grid point locations")

			.def("getIndexMaps",(std::vector<std::map<int,int>> (spaceInterp_multi_exp_complex::*)())&spaceInterp_multi_exp_complex::getIndexMaps,"Get maps from grid locations to regular source traces")
		;
