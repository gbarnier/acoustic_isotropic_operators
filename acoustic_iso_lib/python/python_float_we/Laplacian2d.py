...
class laplacian2d_multi_exp(Op.Operator):
	"""Wrapper encapsulating PYBIND11 module for acoustic wave equation"""
	...
	def __init__(self,domain,range):
		...
		if("getCpp" in dir(range)):
			range = range.getCpp()
		self.pyOp = pyLaplacian2d.Laplacian2d_multi_exp(domain,range)
		return
...
class Laplacian2d_multi_exp_complex(Op.Operator):
	"""Wrapper encapsulating PYBIND11 module for acoustic wave equation"""
	...
	def __init__(self,domain,range):
		...
		if("getCpp" in dir(range)):
			range = range.getCpp()
		self.pyOp = pyLaplacian2d.Laplacian2d_multi_exp_complex(domain,range)
		return
