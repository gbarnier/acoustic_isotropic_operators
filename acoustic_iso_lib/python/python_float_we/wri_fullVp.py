...
if __name__ == '__main__':
	...
	if(inversionMode=="time"):
		p_model,priorFloat,slsqFloat,parObject,waveEquationAcousticOp = Acoustic_iso_float_we.waveEquationOpInitFloat_time(sys.argv)
		# timeMask=0;
		...
		p_model,priorFloat,slsqFloat,parObject,waveEquationAcousticOpTemp,_,modelFloatTime = Acoustic_iso_float_we.waveEquationOpInitFloat_freq(sys.argv)
	...
	for iteration in range(restartIter,nIter):
		...
		if(pFinished==0):
			...
			print("in wri full vp p_solver.inv_model.norm()" + str(current_p_model.norm()))
		pFinished=0
