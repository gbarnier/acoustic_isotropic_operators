...
#	waveEquationAcousticOp = pyOp.ChainOperator(tempWaveEquationOp,mask3dOp)
	...
	#init K
	_,p_dataFloat,dataSamplingOp= wriUtilFloat.data_extraction_op_init(sys.argv)
	...
	if(pyinfo):
		...
		print("p shape: ", p_modelFloat.getNdArray().shape)
		...
		print("d axis 1 sampling: ", p_dataFloat.getHyper().getAxis(1).d)
		print("* Amp - f * ")
	...
	if (initial_p_model_file=="None"):
		...
		current_p_model=p_modelFloat.clone()
		current_p_model.scale(0.0)
	else:
		if(pyinfo): print("Initial wavefield: ", initial_p_model_file)
	...
	if (initial_m_model_file=="None"):
		...
		current_m_model=m_modelFloat.clone()
