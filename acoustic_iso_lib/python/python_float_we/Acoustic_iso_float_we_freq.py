...
def waveEquationOpInitFloat_multi_exp_freq(args):
	...
	weOp=waveEquationAcousticCpu_multi_exp_freq(modelFloat,dataFloat,slsqFloat,U0,alpha,spongeWidth)
...
class waveEquationAcousticCpu_multi_exp_freq(Op.Operator):
	...
	def __init__(self,domain,range,slsqFloat,U0,alpha,spongeWidth):
		...
		if("getCpp" in dir(range)):
			range = range.getCpp()
		self.pyOp = pyAcoustic_iso_float_we_freq.WaveRecon_freq_multi_exp(domain,range,slsqFloat,U0,alpha,spongeWidth)
		return
