...
PYBIND11_MODULE(pyAcoustic_iso_double_nl, clsGeneric) {
  ...
  py::class_<deviceGpu, std::shared_ptr<deviceGpu>>(clsGeneric, "deviceGpu")
      .def(py::init<const std::shared_ptr<SEP::double1DReg>, const std::shared_ptr<SEP::double1DReg>, const std::shared_ptr<double2DReg>, int &, int, double, double>(), "Initialize a deviceGPU object using location, velocity, and nt")

      .def(py::init<const std::vector<int> &, const std::vector<int> &, const std::shared_ptr<double2DReg>, int &, int , int, int>(), "Initlialize a deviceGPU object using coordinates and nt")

      .def(py::init<const int &, const int &, const int &, const int &, const int &, const int &, const std::shared_ptr<double2DReg>, int &, int, int, int>(), "Initlialize a deviceGPU object using sampling in z and x axes, velocity, and nt")
